...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None, logger=None):
        ...
        space_fmt = ':' + str(6) + 'd' # avoid iterable wo __len__
...
# def nested_tensor_from_tensor_list(tensor_list: List[Tensor], size_divisibility=32, split=False):
#     if split:
...
def collate_fn_tta(batch):
    batch = sum(batch, [])
    batch = list(zip(*batch))    
    # batch[0] = [nested_tensor_from_tensor_list([batch_elem]) for batch_elem in batch[0]]
    batch[0] = nested_tensor_from_tensor_list(batch[0])
    return tuple(batch)