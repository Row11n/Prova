...
class ContentAttn(nn.Module):
    ...
    def forward(self, content_query, label_embedding):
        content_query2 = self.attn(content_query, label_embedding, label_embedding)[0]
        content_query = content_query + self.dropout(content_query2)
        content_query = self.norm(content_query)
...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
        ...
        self.attn_label_enc = attn_label_enc
        self.label_enc = None
        if attn_label_enc :
            self.label_attn = ContentAttn(d_model, nhead, dropout)
        assert query_dim == 4
        ...
        self.decoder = TransformerDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                        ...
                                        calibrate_boxes=calibrate_boxes
                                        )
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None, box2query=None):
        """
        Input:
        ...
        for src in srcs :
            h, w = src.shape[-2:]
            src_mem = memory[:, start_idx:start_idx+h*w]
            src_mem = src_mem.transpose(1,2).reshape(bs, c, h, w)
        ...
        if self.two_stage_type =='standard':
            ...
            if self.embed_init_tgt:
                ...
                if self.attn_label_enc :
                    label_enc = self.label_enc.weight[:, None, :].repeat(1, bs, 1).transpose(0, 1)
                    tgt_ = self.label_attn(tgt_.transpose(0, 1), label_enc.transpose(0, 1)).transpose(0, 1)
        ...
        # Begin Decoder
        #########################################################
        if box2query is not None :
            tgt = box2query(refpoint_embed.sigmoid()) + 0.0 * tgt # avoid unused params
        ...
        return hs, references, hs_enc, ref_enc, init_box_proposal, src_mems
        # hs: (n_dec, bs, nq, d_model)
...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm=None, 
        ...
        self.num_feature_levels = num_feature_levels
        self.use_detached_boxes_dec_out = use_detached_boxes_dec_out
        self.calibrate_boxes = calibrate_boxes
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder:
                ...
                query_sine_embed = gen_sineembed_for_position(reference_points_input[:, :, 0, :], self.d_model // 2) # nq, bs, 256*2 
            else:
                query_sine_embed = gen_sineembed_for_position(reference_points, self.d_model // 2) # nq, bs, 256*2
...
def build_deformable_transformer(args):
    ...
    try:
        ...
        use_detached_boxes_dec_out =False
    try :
        calibrate_boxes = args.calibrate_boxes
    except :
        calibrate_boxes = False
    ...
    return DeformableTransformer(
        ...
        attn_label_enc=args.attn_label_enc,
